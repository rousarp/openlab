...
function openlab_custom_the_content($content) {
    ...
    if ($post->post_name == 'contact-us' && ($post->post_type == 'page' || $post->post_type == 'help')) {
        $form = do_shortcode('[contact-form-7 id="53" title="Contact Form 1"]');
        $content = <<<HTML
                <div class="panel panel-default">
                    <div class="panel-heading">Kontaktní formulář</div>
...
function openlab_custom_menu_items($items, $menu) {
    ...
    if ($menu->theme_location == 'main') {
        ...
        if (is_user_logged_in()) {
            ...
            $opl_link = '<li ' . $class . '>';
            $opl_link .= '<a href="' . bp_loggedin_user_domain() . '">Můj OpenLab</a>';
            $opl_link .= '</li>';
        }
