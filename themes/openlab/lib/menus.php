...
function openlab_my_friends_submenu($count = true) {
    ...
    if ($bp->is_item_admin) {
        $menu_list = array(
            $friend_requests => 'Přijaté žádosti ' . $count_span,
                //'#' => $page_identify,
        );
    } else {
        return '';
...
function openlab_group_admin_tabs($group = false) {
    ...
    <?php if (openlab_is_portfolio()) : ?>
      ...
      --><li class="delete-button <?php if ('delete-group' == $current_tab) : ?> current-menu-item<?php endif; ?>" ><span class="fa fa-minus-circle"></span><a href="<?php echo bp_get_root_domain() . '/' . bp_get_groups_root_slug() . '/' . $group->slug ?>/admin/delete-group">Smazat <?php echo $profile; ?></a></li><!--
