...
function wds_email_validate() {
	...
	if ( ! defined( 'OPENLAB_SKIP_EMAIL_CHECK' ) || ! OPENLAB_SKIP_EMAIL_CHECK ) {
		...
		$account_type = isset( $_POST['field_2'] ) ? stripslashes( $_POST['field_2'] ) : 'Student';
		...
		switch ( $account_type ) {
			...
			case 'Non-City Tech':
				...
				if ( ! cac_ncs_validate_code( $code ) ) {
					$bp->signup->errors['signup_email'] = 'E-mailové adresy mimo veřejnou správu potřebují pro registraci na portále OpenLab platný registrační kód.';
	...
	if ( in_array('citytech', $domains) && in_array('cuny', $domains) && in_array('edu', $domains) ) {
		echo 'No error!';
		$error = 0;
	}elseif ( in_array('mail', $domains) && in_array('citytech', $domains) && in_array('cuny', $domains) && in_array('edu', $domains) && $_POST['field_2']=="Student") {
		echo 'No error!';
		$error = 0;
			...
			while ( bp_profile_groups() ) :
				...
				while ( bp_profile_fields() ) :
					...
					if ( 'textbox' == bp_get_the_profile_field_type() ) :
						...
						$return .= "<br />Post Field 193: " . $_POST['field_9'];
