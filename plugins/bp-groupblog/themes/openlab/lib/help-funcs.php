...
function openlab_help_loop() {
    ...
    while ($hp_query->have_posts()) : $hp_query->the_post();
        ...
        if ($next_post) {
            $back_next_nav .= '--><span class="nav-previous">';
            $back_next_nav .= sprintf('<a href="%s">Další</a>', esc_url(get_permalink($next_post)));
...
function openlab_help_tags_loop() {
    ...
    <h1 class="parent-cat entry-title"><span class="profile-name hyphenate">Záznamy pro šítek: "<?php echo $parent_cat_name; ?>"</span>
        <?php echo openlab_toggle_button('#sidebar-menu-wrapper', true); ?>
    </h1>
